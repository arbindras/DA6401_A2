...
if __name__ == "__main__":
    ...
    train_dataset_cls = OxfordIIITPetDataset(
    root=DATA_ROOT,
    split="train",
    ...
    persistent_workers=True if cuda_available else False,
    )

    # loader_kwargs = dict(num_workers=4, pin_memory=True, persistent_workers=True)

    train_loader_cls = DataLoader(train_dataset_cls, batch_size=BATCH_SIZE, shuffle=True,  **loader_kwargs)
    ...
    print("mask values:", masks.unique()) # expect {0, 1, 2} before loss (0-based after -1 shift in dataset)
    print("bbox sample:", boxes[0])          # expect [cx, cy, w, h] normalized in [0, 1]
    # =========================
